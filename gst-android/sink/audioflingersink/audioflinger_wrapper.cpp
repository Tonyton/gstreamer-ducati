...
#define LOG_NDEBUG 0
#undef LOG_TAG
#define LOG_TAG "Audio_finger_wrapper"
  			...
  			int streamType, int channelCount, uint32_t sampleRate, int bufferCount)
{
	...
	if (handle == NULL)
	{
		LOGE("xuzhi.Tony : Can't get AudioFlingerDeviceHandle.Audioflinger_device_set false\n");
		return -1;
	}
	if(AUDIO_FLINGER_DEVICE_TRACK(handle)) {
		// bufferCount is not the number of internal buffer, but the internal
		// buffer size 
		/******************************modfily by xuzhi.Tony******************************/
		status = AUDIO_FLINGER_DEVICE_TRACK(handle)->set(streamType, sampleRate, 
				format,(channelCount == 2) ? AudioSystem::CHANNEL_OUT_STEREO : AudioSystem::CHANNEL_OUT_MONO/*, 
				bufferCount*/);
		/**************************************end**************************************/
	...
	{
		status = AUDIO_FLINGER_DEVICE_SINK(handle)->open(sampleRate, 
	...
	if (status != NO_ERROR)
	{
		LOGE("xuzhi.Tony : audioflinger_device_set :status return ERROR");
		return -1;
	}
